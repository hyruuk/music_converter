...
# --- Zenity GUI wizard (launched when no CLI args are provided) ---
launch_gui() {
  # Dialog 1 — Source directory
  SOURCE_DIR=$(zenity --file-selection --directory --title="Select Source Directory" \
    --text="Choose the folder containing your audio files (FLAC, WAV, MP3, AIF, M4A)") || exit 0

  # Dialog 2 — Destination directory
  DEST_DIR=$(zenity --file-selection --directory --title="Select Destination Directory" \
    --text="Choose where converted WAV files will be saved. Directory structure is preserved.") || exit 0

  # Dialog 3 — Settings form
  local settings
  settings=$(zenity --forms --title="Conversion Settings" --separator=$'\n' \
    --text="Configure output format and loudness.\nDefaults are set for <b>maximum device compatibility</b>.\nChange only if you know what you need." \
    --add-combo="Target Loudness" \
      --combo-values="-11 LUFS — Club/DJ playback (Recommended)|-14 LUFS — Streaming standard (Spotify/YouTube)|-9 LUFS — Hot, less dynamic range|-16 LUFS — Broadcast (EBU R128)" \
    --add-combo="True Peak Limit" \
      --combo-values="-1 dBTP — Standard headroom (Recommended)|-2 dBTP — Extra safe, more headroom|-0.5 dBTP — Aggressive, minimal headroom" \
    --add-combo="Sample Rate" \
      --combo-values="44100 Hz — CD quality, max compatibility (Recommended)|48000 Hz — Video/broadcast standard" \
    --add-combo="Bit Depth" \
      --combo-values="16-bit — Universal device support (Recommended)|24-bit — Higher quality, less compatible" \
    --add-combo="Force Mode" \
      --combo-values="No — Skip already converted files (Recommended)|Yes — Re-convert everything") || exit 0

  # Parse the 5 lines of output
  local line_lufs line_tp line_sr line_bd line_force
  line_lufs=$(echo "$settings" | sed -n '1p')
  line_tp=$(echo "$settings" | sed -n '2p')
  line_sr=$(echo "$settings" | sed -n '3p')
  line_bd=$(echo "$settings" | sed -n '4p')
  line_force=$(echo "$settings" | sed -n '5p')

  # Apply selections (only override if non-empty, i.e. user picked something)
  if [ -n "$line_lufs" ]; then
    TARGET_LUFS=$(extract_number "$line_lufs")
  fi
  if [ -n "$line_tp" ]; then
    TARGET_TP=$(extract_number "$line_tp")
  fi
  if [ -n "$line_sr" ]; then
    TARGET_SAMPLE_RATE=$(extract_number "$line_sr")
  fi
  if [ -n "$line_bd" ]; then
    TARGET_BIT_DEPTH=$(extract_number "$line_bd")
  ...
  # Dialog 4 — Confirmation
  zenity --question --title="Ready to Convert" --width=500 \
    --text="<b>Source:</b> $SOURCE_DIR\n<b>Destination:</b> $DEST_DIR\n\n<b>Format:</b> WAV ($TARGET_CODEC, ${TARGET_SAMPLE_RATE} Hz, ${TARGET_BIT_DEPTH}-bit, stereo)\n<b>Loudness:</b> ${TARGET_LUFS} LUFS, TP: ${TARGET_TP} dBTP\n<b>Force:</b> $FORCE_MODE\n\nProceed?" || exit 0
}
...
log() {
  ...
  if [ -n "$LOG_FILE" ]; then
    echo -e "$(date '+%Y-%m-%d %H:%M:%S') - ${log_level} - ${message}" >> "$LOG_FILE"
  fi

  if [ "$log_level" = "ERROR" ] || [ "$console_output" = "force" ]; then
    echo -e "${color}${log_level}: ${message}${C_NC}"
  fi
}
...
update_progress_bar() {
  ...
  echo -ne "\033[2A" # Move cursor up two lines
  echo -ne "\r\033[K" # Clear current line
  echo -e "${color}Total progress: ${percentage}% ${progress_bar} (${processed_files}/${total_files})${C_NC}"
  echo -ne "\r\033[K" # Clear current line
  echo -e "${color}Current file: $(basename "$current_file")${C_NC}"
}
